...
const nextHandler = (slider, slideWidth, fullWidth) => {
  ...
  if (scrollX + slideWidth < fullWidth) {
    slider.querySelector('.slider-slides').scrollTo({top: 0, left: scrollX + slideWidth, behavior: 'smooth'});
  } else {
    slider.querySelector('.slider-slides').scrollTo({top: 0, left: 0, behavior: 'smooth'});
  }
}
const prevHandler = (slider, slideWidth, fullWidth) => {
  let scrollX = slider.querySelector('.slider-slides').scrollLeft;
  if (scrollX == 0) {
    slider.querySelector('.slider-slides').scrollTo({top: 0, left: fullWidth - slideWidth, behavior: 'smooth'});
  } else {
    slider.querySelector('.slider-slides').scrollTo({top: 0, left: scrollX - slideWidth, behavior: 'smooth'});
  }
}
